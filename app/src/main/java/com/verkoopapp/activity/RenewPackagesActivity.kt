...
class RenewPackagesActivity : AppCompatActivity(), AdvertPackageAdapter.SubmitBannerCallBack {
        ...
        ServiceHelper().getAdvertisementPlanService(Utils.getPreferencesString(this,AppConstants.USER_ID).toInt(),object : ServiceHelper.OnResponse {
            override fun onSuccess(response: Response<*>) {
                ...
                if (responseWallet.data.isNotEmpty()) {
                    ...
                    Utils.showSimpleMessage(this@RenewPackagesActivity, "No data found.").show()
                }
            }

            override fun onFailure(msg: String?) {
                pbAdvertPackage.visibility = View.GONE
                Utils.showSimpleMessage(this@RenewPackagesActivity, msg!!).show()
            }
        })
    ...
    private fun purchaseAdvertisementApi(planId: Int, coin:Int) {
        ...
        ServiceHelper().updateBannerService(uploadBannerRequest, object : ServiceHelper.OnResponse {
            override fun onSuccess(response: Response<*>) {
                ...
                Utils.showToast(this@RenewPackagesActivity, homeDataResponse.message)
                setDialogBox()
                if(coin<=Utils.getPreferencesInt(this@RenewPackagesActivity, AppConstants.COIN)){
                val  remainingCoin=  Utils.getPreferencesInt(this@RenewPackagesActivity, AppConstants.COIN)-coin
                    Utils.saveIntPreferences(this@RenewPackagesActivity,AppConstants.COIN,remainingCoin)
                }
    ...
    private fun setDialogBox() {
        val shareDialog = WarningDialog(this, object : SelectionListener {
            override fun leaveClick() {
                val intentBack = Intent(this@RenewPackagesActivity, ViewAllBannerActivity::class.java)
